...
import { Loader, Modal, Realized } from '../components'
...
const PromptCard = ({ post, handleTagClick, handleEdit, handleDel }) => {
    ...
    console.log(parseInt(post.realizedDate) - parseInt(post.createdOn))
    console.log('post.createdOn; ', parseInt(post.createdOn))
    console.log('realizedDate:', parseInt(post.realizedDate))
    ...
    const handleUnrealized = async (realized) => {
        setLoading(true)
        const response = await fetch(`/api/realized`, {
            method: 'PATCH',
            headers: {
    ...
    useEffect(() => {
        if (post.realized) {
            ...
            if (parseInt(post.createdOn) - parseInt(post.realizedDate) === 3) {
                try {
    ...
    if (loading) {
        return (
            ...
            >
                <div className={'absolute bottom-0 right-5 z-10 cursor-pointer'}><Loader /></div>
                <div
                    className={'blur-sm relative'}
                >
        ...
        <>
            {realizedPost ? (
                <div
                    ...
                    onClick={() => { handleUnrealized(false), setRealizedPost(false) }}
                >
